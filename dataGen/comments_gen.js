...
async function createComments() {
  const writeStream = fs.createWriteStream('dataGen/data/comments.csv');
  writeStream.write('id, author, avatar, time, date, text, song_id \n');
  for (let i = 1; i <= 400000000; i += 1) {
    const ableToWrite = writeStream.write(`${i},${getName()},${getAvatar()},${getTime()},${getDate()},${getText()},${getsongId()}\n`);
    if (!ableToWrite) {
